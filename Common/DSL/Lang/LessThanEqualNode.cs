...
namespace DSL.Lang {
    public class LessThanEqualNode : BoolLogicExpressionNode  {
        public LessThanEqualNode(Token token, ExpressionNode inLeft, ExpressionNode inRight)
            : base(token) {
            SetLeft(inLeft);
            SetRight(inRight);
        }
        ...
        public override void Eval(IExecutionContext context) {
            left.Eval(context);
            right.Eval(context);
            if (context.LessThanEqual(left, right)) {
                SetResult(true);
            } else {
                SetResult(false);
            }
