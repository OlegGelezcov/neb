...
namespace DSL.Lang {
    public class LessThanNode :  BoolLogicExpressionNode {

        public LessThanNode(Token token, ExpressionNode inLeft, ExpressionNode inRight)
            : base(token) {
            SetLeft(inLeft);
            SetRight(inRight);
        }
        ...
        public override void Eval(IExecutionContext context) {
            left.Eval(context);
            right.Eval(context);
            if(context.LessThan(left, right)) {
                SetResult(true);
            } else {
                SetResult(false);
            }
