...
namespace nebula_console_test {
    public class Aes {
        ...
        private byte[,] state;
        private readonly byte[] RoundKey = new byte[176];
        ...
        private readonly byte[] rsbox = new byte[256] {
        ...
        private readonly byte[] Rcon = new byte[255] {
        ...
        private byte getSBoxValue(byte num) {
            return sbox[num];
        }
        private byte getSBoxInvert(byte num) {
            return rsbox[num];
        }
        ...
        private void AddRoundKey(byte round) {
            ...
            for(i = 0; i < 4; ++i ) {
                for(j = 0; j < 4; ++j ) {
                    state[i, j] = (byte)(state[i, j] ^ RoundKey[round * Nb * 4 + i * Nb + j]);
                }
            }
        }
        ...
        void BlockCopy(byte[,] output, byte[,] input) {
            for(int i = 0; i < 4; ++i ) {
                for(int j = 0; j < 4; ++j ) {
        ...
        private void AES128_ECB_encrypt(byte[,] input, byte[] key, byte[,] output) {
        ...
        private void AES128_ECB_decrypt(byte[,] input, byte[] key, byte[,] output) {
            BlockCopy(output, input);
            state = output;
            Key = key;
            KeyExpansion();
