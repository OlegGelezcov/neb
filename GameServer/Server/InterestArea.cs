...
namespace Space.Server
{
    ...
    public class InterestArea : IDisposable
    {
        ...
        /// </param>
        public InterestArea(byte id, IWorld world)
        {
            this.id = id;
            this.world = world;
        ...
        public void AttachToItem(Item item)
        {
            ...
            this.Position = item.transform.position.ToVector();
        ...
        public void UpdateInterestManagement()
        {
            ...
            if (innerFocus != this.currentRegionInnerFocus)
            {
                if (innerFocus.IsValid())
                {
                    ...
                    //ConsoleLogging.Get.Print("number of new regions in inner focus: {0}", regions.Count);
